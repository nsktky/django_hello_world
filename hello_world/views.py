...
# functionベースビューでの作成。httpレスポンスをurls.pyで受け取って、この関数にrequestオブジェクトを渡している
# 渡されたrequestオブジェクトに対し、httpresponseでオブジェクトをブラウザに返す
# functionベースビューで作成すると、自分でコードを組み立てる必要があるが、細かい調整等がしやすい
def hello_world_fanction(request):
    returned_object = HttpResponse('<h1>hello world</h1>')
    return returned_object

# classベースビューでの作成。必要なクラスを承継して使用。今回はTemplateViewを承継
